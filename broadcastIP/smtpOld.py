...
# Account Information
to = 'krptx356951@gmail.com' # Email to send to.
gmail_user = 'roboieeeucsd@gmail.com' # Email to send from. (MUST BE GMAIL)
gmail_password = 'teamworK15keY9' # Gmail password.
...
split_line = []
#print "ip_lines:  ", ip_lines, "\n", "lngth: ", len(ip_lines)
for x in range(len(ip_lines)):
	split_line.append(ip_lines[x].split())
#print "split_line:", split_line, "\n"

# con_type variables for the message text. ex) 'ethernet', 'wifi', etc.
ip_type = []
for x in range(len(ip_lines)):
	ip_type.append(connect_type(split_line[x]))
...
"""
#Problems for the first element in the outer list because it doesn't contain src
for x in range(len(ip_lines)):
	ipaddr.append(split_line[split_line[x].index('src')+1])
